...
?><div id="body-container">
	...
	if(strtolower($rr->action) != "delete") 
	{
		...
		?><div class="self-container">
			...
			if($is_linked)
			{ 
			?><p>This Object is linked elsewhere, so the original will not be deleted.</p><?php
			}
			else
			{
			?><p>Warning! You are about to permanently delete this Object.</p><?php
				if($k) 
				{ 
			?><p>The following <?php
					if($k > 1)
						...
						echo "Object"; 
					?> will also be deleted as a result:</p><?php
		...
		?><form 
				action="<?= $admin_path.'delete/'.$uu->urls() ?>" 
				method="post"
			>
